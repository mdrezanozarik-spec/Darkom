...
do_uninstall() {
  ...
  echo -e "${CYAN}======================================================${NC}"
  echo -e "${YELLOW}Note:${NC} Firewall rules for port 55 (UDP/TCP) were left in place."
...
check_port55() {
  ss -H -lun "sport = :55" 2>/dev/null | grep -q ':55' && return 0
  ss -H -ltn "sport = :55" 2>/dev/null | grep -q ':55' && return 0
  return 1
}

show_port55_usage() {
  log_warn "Current listeners on port 55:"
  ss -lupn "sport = :55" 2>/dev/null || true
  ss -ltpn "sport = :55" 2>/dev/null || true
  lsof -nP -iUDP:55 -iTCP:55 2>/dev/null || true
}

get_port55_pids() {
  local pids_udp pids_tcp pids
  pids_udp="$(ss -H -lupn "sport = :55" 2>/dev/null | sed -n 's/.*pid=\([0-9]\+\).*/\1/p' | sort -u)"
  pids_tcp="$(ss -H -ltpn "sport = :55" 2>/dev/null | sed -n 's/.*pid=\([0-9]\+\).*/\1/p' | sort -u)"
  pids="$(printf '%s\n%s\n' "$pids_udp" "$pids_tcp" | sed '/^$/d' | sort -u)"
  if [[ -n "$pids" ]]; then
    echo "$pids"
    return 0
  fi
  lsof -ti :55 2>/dev/null || true
}
...
terminate_port55_pid() {
  ...
  local cmdline
  cmdline="$(ps -p "$pid" -o cmd= 2>/dev/null || true)"
  log_warn "Trying to terminate PID on port 55: $pid (${cmdline:-unknown})"
...
force_release_port55() {
  local stubborn=0
  local pid

  while IFS= read -r pid; do
    [[ -z "$pid" ]] && continue
    terminate_port55_pid "$pid" || stubborn=1
  done <<< "$(get_port55_pids)"

  if command -v fuser >/dev/null 2>&1 && check_port55; then
    log_warn "Trying fuser fallback for port 55..."
    fuser -k 55/udp 2>/dev/null || true
    fuser -k 55/tcp 2>/dev/null || true
    sleep 1
  fi
...
remove_iptables_port55_redirects() {
  local tool="$1"
  ...
  while IFS= read -r rule; do
    ...
    log_warn "Removing ${tool} NAT redirect rule for port 55: $rule"
    # shellcheck disable=SC2086
    $tool -t nat $delete_rule >/dev/null 2>&1 || true
  done < <("$tool" -t nat -S 2>/dev/null | grep -E '(^-A )' | grep -E -- '(-p (tcp|udp)|-p (udp|tcp)).*--dport 55([^0-9]|$)' | grep -E 'REDIRECT|DNAT' || true)
}

remove_nft_port55_redirects() {
  command -v nft >/dev/null 2>&1 || return 0

  local rule
  while IFS= read -r rule; do
    [[ -z "$rule" ]] && continue
    log_warn "Removing nftables redirect rule for port 55: $rule"
    nft delete rule $rule >/dev/null 2>&1 || true
  done < <(nft -a list ruleset 2>/dev/null | awk '
    / dport 55 / && ($0 ~ /redirect/ || $0 ~ /dnat/) {
      for (i = 1; i <= NF; i++) {
...
remove_port55_forward_rules() {
  log_info "Checking for port 55 redirect/forward rules..."
  remove_iptables_port55_redirects iptables
  remove_iptables_port55_redirects ip6tables
  remove_nft_port55_redirects
}
...
stop_existing_stormdns_service() {
  ...
  if systemctl list-unit-files --all 2>/dev/null | grep -q '^stormdns\.service'; then
    ...
    if [[ -n "${main_pid:-}" && "$main_pid" != "0" ]] && kill -0 "$main_pid" 2>/dev/null; then
      ...
      terminate_port55_pid "$main_pid" || true
    fi
...
log_header "Managing Network Ports (Port 55)"
remove_port55_forward_rules

if check_port55; then
  log_warn "Port 55 is occupied. Trying auto-cleanup..."
  show_port55_usage
  ...
  if check_port55; then
    log_warn "Port 55 is still busy after stopping known services. Trying direct process termination..."
    force_release_port55 || true
  fi

  if check_port55 && systemctl is-active --quiet systemd-resolved; then
    log_warn "Port 55 is still in use. Stopping systemd-resolved completely..."
  ...
  if check_port55; then
    log_warn "Port 55 still occupied. Trying one more forced cleanup pass..."
    force_release_port55 || true
  fi

  if check_port55; then
    OCC_INFO="$(ss -H -lupn 'sport = :55' 2>/dev/null | head -n1 | awk '{print $NF}' || true)"
    [[ -z "${OCC_INFO:-}" ]] && OCC_INFO="$(ss -H -ltn 'sport = :55' 2>/dev/null | head -n1 | awk '{print $NF}' || true)"
    show_port55_usage
    log_error "Port 55 is still occupied: ${OCC_INFO:-unknown}. Stop it manually and retry."
  fi
fi
log_success "Port 55 is available."

log_header "Configuring Firewall (Port 55 UDP/TCP)"
ACTIVE_FIREWALL="none"
if command -v ufw >/dev/null 2>&1 && ufw status | grep -qw active; then
  ACTIVE_FIREWALL="ufw"
  ufw allow 55/udp >/dev/null 2>&1 || true
  ufw allow 55/tcp >/dev/null 2>&1 || true
  log_success "Port 55 (UDP/TCP) opened via UFW."
elif command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld; then
  ACTIVE_FIREWALL="firewalld"
  firewall-cmd --permanent --add-port=55/udp >/dev/null 2>&1 || true
  firewall-cmd --permanent --add-port=55/tcp >/dev/null 2>&1 || true
  firewall-cmd --reload >/dev/null 2>&1 || true
  log_success "Port 55 (UDP/TCP) opened via firewalld."
elif command -v iptables >/dev/null 2>&1; then
  ACTIVE_FIREWALL="iptables"
  iptables -C INPUT -p udp --dport 55 -j ACCEPT 2>/dev/null || iptables -I INPUT -p udp --dport 55 -j ACCEPT
  iptables -C INPUT -p tcp --dport 55 -j ACCEPT 2>/dev/null || iptables -I INPUT -p tcp --dport 55 -j ACCEPT
  if command -v ip6tables >/dev/null 2>&1; then
    ip6tables -C INPUT -p udp --dport 55 -j ACCEPT 2>/dev/null || ip6tables -I INPUT -p udp --dport 55 -j ACCEPT
    ip6tables -C INPUT -p tcp --dport 55 -j ACCEPT 2>/dev/null || ip6tables -I INPUT -p tcp --dport 55 -j ACCEPT
  fi
  ...
  log_success "Port 55 (UDP/TCP) rule is ready via iptables."
elif command -v nft >/dev/null 2>&1; then
  ACTIVE_FIREWALL="nftables"
  if nft list table inet filter >/dev/null 2>&1; then
    nft add rule inet filter input udp dport 55 accept >/dev/null 2>&1 || true
    nft add rule inet filter input tcp dport 55 accept >/dev/null 2>&1 || true
    log_success "Port 55 (UDP/TCP) rule is ready via nftables."
  else
    log_warn "nftables is present but no 'inet filter' table was found. Open port 55 manually if needed."
  fi
else
...
net.ipv4.udp_mem = 65556 131072 262144
net.netfilter.nf_conntrack_max = 262144
net.netfilter.nf_conntrack_udp_timeout = 15
net.netfilter.nf_conntrack_udp_timeout_stream = 60
net.ipv4.ip_local_port_range = 10240 65555
EOF
...
if [[ "$READY" != true ]]; then
  ...
  log_error "Could not verify key generation. Ensure Port 55 is free."
fi
